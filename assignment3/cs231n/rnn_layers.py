...
def rnn_step_backward(dnext_h, cache):
  ...
  dx = np.dot(dSum, Wx.T)
  dprev_h = np.dot(dSum, Wh.T)
  dWx = np.dot(x.T, dSum)
  dWh = np.dot(prev_h.T, dSum)
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(x.shape[0]):
    ...
    h[t,:,:] = next_h
    cache['step_cache'].append(step_cache)
...
def rnn_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    dht = dh[:,t,:].reshape(N,H) + dht
    dxt, dht, dWxt, dWht, dbt = rnn_step_backward(dht, cache['step_cache'][t])
    if dx is None:
      dx = np.zeros([dxt.shape[0], T, dxt.shape[1]])
    dx[:,t,:] = dxt
    dWx = (dWxt if dWx is None else dWx + dWxt)
    dWh = (dWht if dWh is None else dWh + dWht)
    db = (dbt if db is None else db + dbt)
...
def word_embedding_backward(dout, cache):
  ...
  flat_dout = dout.reshape(-1, dout.shape[2])

  np.add.at(dW, flat_x, flat_dout)
  return dW
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  dtanh_next_c = dnext_h * cache['o']
  dnext_c += dtanh_next_c * (1 - cache['tanh_next_c'] ** 2)
  dprev_c = dnext_c * cache['f']
  
  do = dnext_h * cache['tanh_next_c']
  
  df = dnext_c * cache['prev_c']
  di = dnext_c * cache['g']
  dg = dnext_c * cache['i']

  dai = di * cache['i'] * (1 - cache['i'])
  daf = df * cache['f'] * (1 - cache['f'])
  dao = do * cache['o'] * (1 - cache['o'])
  dag = dg * (1 - cache['g'] ** 2)

  da = np.concatenate(
    (np.concatenate((dai, daf), axis=1), np.concatenate((dao, dag), axis=1)),
    axis = 1)

  db = np.sum(da, axis=0)
  #a = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
  dx = np.dot(da, cache['Wx'].T)
  dWx = np.dot(cache['x'].T, da)
  dprev_h = np.dot(da, cache['Wh'].T)
  dWh = np.dot(cache['prev_h'].T, da)
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    dht = dh[:,t,:].reshape(N,H) + dht
    dxt, dht, dct, dWxt, dWht, dbt = lstm_step_backward(dht, dct, cache['step_cache'][t])
    if dx is None:
      dx = np.zeros([dxt.shape[0], T, dxt.shape[1]])
    dx[:,t,:] = dxt
    dWx = (dWxt if dWx is None else dWx + dWxt)
