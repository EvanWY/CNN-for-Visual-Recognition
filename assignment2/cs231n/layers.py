...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  momentum = bn_param.get('momentum', 0.4)
  ...
  if mode == 'train':
    ...
    sample_mean = np.mean(x, axis=0)
    temp_running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    delta_x = x - temp_running_mean.reshape(1,-1)

    sqr_delta_x = delta_x ** 2
    sample_var = np.mean(sqr_delta_x, axis=0)
    temp_running_var = momentum * running_var + (1 - momentum) * sample_var
    norm_fraction = (eps + temp_running_var) ** (-0.5)
    norm_x = delta_x * norm_fraction.reshape(1,-1)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  N, C, H, W = x.shape
  reshaped_x = np.moveaxis(x, 1, 0).reshape(C, -1).T
  reshaped_out, cache = batchnorm_forward(reshaped_x, gamma, beta, bn_param)
  out = np.moveaxis(reshaped_out.T.reshape(C, N, H, W), 1, 0)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  # be very short; ours is less than five lines.                              #
  N, C, H, W = dout.shape
  reshaped_dout = np.moveaxis(dout, 1, 0).reshape(C, -1).T
  reshaped_dx, dgamma, dbeta = batchnorm_backward(reshaped_dout, cache)
  dx = np.moveaxis(reshaped_dx.T.reshape(C, N, H, W), 1, 0)
